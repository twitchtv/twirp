...
func TestChainClientHooks(t *testing.T) {
	var (
		hook1 = new(twirp.ClientHooks)
		hook2 = new(twirp.ClientHooks)
		hook3 = new(twirp.ClientHooks)
	...
	hook2.Error = func(ctx context.Context, twerr twirp.Error) {
		errorCalled = append(errorCalled, "hook2")
	}

	chain := twirp.ChainClientHooks(hook1, hook2, hook3)
