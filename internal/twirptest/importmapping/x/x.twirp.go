...
func NewSvc1ProtobufClient(baseURL string, client HTTPClient, opts ...twirp.ClientOption) Svc1 {
	...
	serviceURL += "/twirp.internal.twirptest.importmapping.x.Svc1"
	urls := [1]string{
		serviceURL + "/Send",
	}
...
// Should be used with caution, because it only matches routes generated by
// Twirp Go clients if the service is mounted on the default "/twirp" prefix.
...
func (s *svc1Server) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	if len(parts) < 2 || parts[len(parts)-2] != "twirp.internal.twirptest.importmapping.x.Svc1" {
