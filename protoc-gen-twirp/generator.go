...
func (t *twirp) generateStreamUtils() {
	...
	t.P(`	)`)
	t.P(``)
	t.P(`	if tag != msgTag && tag != trailerTag {`)
	...
	t.P(`		_, err = io.ReadFull(r.r, buf)`)
	t.P(`		if err != nil {`)
	t.P(`			return err`)
	t.P(`		}`)
	t.P()
	t.P(`		err = proto.Unmarshal(buf, msg)`)
	t.P(`		if err != nil {`)
	t.P(`			return err`)
	t.P(`		}`)
	t.P(`		return nil`)
	t.P(`	}`)
	t.P()
	t.P(`	// This is a trailer, read it and then close the client`)
	t.P(`	defer r.c.Close()`)
	t.P(`	buf := make([]byte, int(l))`)
	...
	t.P(`	}`)
	t.P()
	t.P(`	// Put the length back in front of the trailer so it can be decoded`)
	t.P(`	buf = append(proto.EncodeVarint(l), buf...)`)
	t.P(`	var trailer string`)
	t.P(`	trailer, err = `, t.pkgs["proto"], `.NewBuffer(buf).DecodeStringBytes()`)
	t.P(`	if err != nil {`)
	t.P(`		return clientError("failed to read stream trailer", err)`)
	t.P(`	}`)
	t.P(`	if trailer == "EOF" {`)
	t.P(`		return io.EOF`)
	t.P(`	}`)
	t.P(`	var tj twerrJSON`)
	t.P(`	if err = json.Unmarshal([]byte(trailer), &tj); err != nil {`)
	t.P(`		return clientError("unable to decode stream trailer", err)`)
	t.P(`	}`)
	t.P(`	return tj.toTwirpError()`)
	t.P(`}`)
	t.P()
...
func (t *twirp) generateServerProtobufMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	if rpcType == unary {
		t.P(`	var err error`)
		t.P(`	writeProtoError := func(err error) {`)
		t.P(`		s.writeError(ctx, resp, err)`)
		t.P(`	}`)
	}
	if rpcType == download {
		t.P(`	var (`)
		t.P(`		err        error`)
		t.P(`		respStream `, t.methodOutputType(method))
		t.P(`	)`)
		t.P()
		t.P(`	// Prepare trailer`)
		t.P(`	trailer := proto.NewBuffer(nil)`)
		t.P(`	_ = trailer.EncodeVarint((2 << 3) | 2) // field tag`)
		t.P(`	writeProtoError := func(err error) {`)
		t.P(`		// JSON encode err as twirp err`)
		t.P(`		// TODO: figure out what to do about updating context and headers`)
		t.P(`		if err == io.EOF {`)
		...
		t.P(`		}`)
		t.P(`		_ = trailer.EncodeStringBytes(`)
		t.P(`			string(marshalErrorToJSON(twerr)),`)
		t.P(`		)`)
		t.P(`	}`)
		t.P(`	defer func() { // Send trailer`)
		t.P(`		_, err = resp.Write(trailer.Bytes())`)
		t.P(`		if err != nil {`)
		t.P(`			// TODO: call error hook?`)
		t.P(`			err = wrapErr(err, "failed to write trailer")`)
		t.P(`			respStream.End(err)`)
		t.P(`		}`)
		t.P(`	}()`)
	}
	...
	if rpcType == unary || rpcType == download {
		...
		t.P(`    writeProtoError(`, t.pkgs["twirp"], `.InternalErrorWith(err))`)
		t.P(`    return`)
		t.P(`  }`)
		t.P(`  reqContent := new(`, t.methodInputType(method), `)`)
	...
	if rpcType == download {
		...
		t.P(`			// In case of a panic, serve a 500 error and then panic.`)
		t.P(`			if r := recover(); r != nil {`)
		t.P(`				writeProtoError(`, t.pkgs["twirp"], `.InternalError("Internal service panic"))`)
		t.P(`				panic(r)`)
		t.P(`			}`)
		t.P(`		}()`)
		t.P(`		respStream, err = s.`, methName, `(ctx, reqContent)`)
		t.P(`	}()`)
		t.P(``)
		t.P(`	if err != nil {`)
		t.P(`		writeProtoError(err)`)
		...
		t.P(`	}`)
		t.P(``)
		t.P(`	if respStream == nil {`)
		t.P(`		writeProtoError(`, t.pkgs["twirp"], `.InternalError("received a nil `, t.methodInputType(method), ` and nil error while calling `, methName, `. nil responses are not supported"))`)
		t.P(`		return`)
		t.P(`	}`)
		t.P(``)
		t.P(`	ctx = callResponsePrepared(ctx, s.hooks)`)
		t.P(``)
		t.P(`	respFlusher, canFlush := resp.(http.Flusher)`)
		t.P(`	messages := `, t.pkgs["proto"], `.NewBuffer(nil)`)
		t.P(`	for {`)
		t.P(`		msg, err := respStream.Next(ctx)`)
		t.P(`		if err != nil {`)
		t.P(`			writeProtoError(err)`)
		t.P(`			break`)
		t.P(`		}`)
		...
		t.P(`		}`)
		t.P(``)
		t.P(`		// TODO: Call a hook that we sent a message in a stream?`)
		t.P(`	}`)
	}
