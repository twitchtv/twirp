...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P()
	t.P(`  defer func() {`)
	t.P(`    closeErr := req.Body.Close()`)
	t.P(`    if err == nil && closeErr != nil {`)
	t.P(`      closeErr = wrapErr(closeErr, "failed to close request body")`)
	t.P(`      callError(ctx, s.hooks, `, t.pkgs["twirp"], `.InternalErrorWith(closeErr))`)
	t.P(`    }`)
	t.P(`  }()`)
	t.P()
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithMethodName(ctx, "`, methName, `")`)
	...
	t.P(`    err = wrapErr(err, "failed to write response to client")`)
	t.P(`    callError(ctx, s.hooks,`, t.pkgs["twirp"], `.InternalErrorWith(err))`)
	t.P(`  }`)
