...
func (t *twirp) generateServer(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	t.P(`  if ctxErr := `, t.pkgs["context"], `.Canceled; ctxErr == ctx.Err() {`)
	t.P(`    s.writeError(ctx, resp, `, t.pkgs["twirp"], `.NewError(`, t.pkgs["twirp"], `.Canceled, ctxErr.Error()))`)
	t.P(`    return`)
	t.P(`  } `)
	t.P(`  if ctxErr := `, t.pkgs["context"], `.DeadlineExceeded; ctxErr == ctx.Err() {`)
	t.P(`    s.writeError(ctx, resp, `, t.pkgs["twirp"], `.NewError(`, t.pkgs["twirp"], `.DeadlineExceeded, ctxErr.Error()))`)
	t.P(`    return`)
	t.P(`  }`)
