...
func (t *twirp) generateServerRouting(servStruct string, file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	t.P(`// Should be used with caution, because it only matches routes generated by`)
	t.P(`// Twirp Go clients if the service is mounted on the default "/twirp" prefix.`)
	...
	t.P(`  parts := strings.Split(req.URL.Path, "/")`)
	t.P(`  if len(parts) < 2 || parts[len(parts)-2] != `, strconv.Quote(fullServiceName(file, service)), ` {`)
	...
	t.P()
	t.P(`  // Route by <Method>`)
	t.P(`  switch parts[len(parts)-1] {`)
	for _, method := range service.Method {
		methSuffix := stringutils.CamelCase(method.GetName())
		methName := "serve" + stringutils.CamelCase(method.GetName())
		t.P(`  case `, strconv.Quote(methSuffix), `:`)
