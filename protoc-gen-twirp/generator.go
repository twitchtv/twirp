...
func (t *twirp) generateUtils() {
	...
	t.P()
	t.P(`  if err = ctx.Err(); err != nil {`)
	t.P(`    if resp.Body != nil {`)
	t.P(`      // make sure the response body is fully read`)
	t.P(`      if _, copyErr := io.Copy(io.Discard, resp.Body); copyErr != nil {`)
	t.P(`	     fmt.Fprintf(os.Stderr, "failed to copy the response body on ctx error: %v\n", copyErr)`)
	t.P(`      }`)
	t.P(`    }`)
	t.P(`    return ctx, wrapInternal(err, "aborted because context was done")`)
	t.P(`  }`)
