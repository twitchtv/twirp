...
func (t *twirp) generateUtils() {
	...
	t.P(`func done(ctx `, t.pkgs["context"], `.Context) error {`)
	t.P(`  select {`)
	t.P(`  case <-ctx.Done():`)
	t.P(`    return ctx.Err()`)
	t.P(`  default:`)
	...
	t.P(`    // Twirp clients dont't follow redirects automatically, Twirp only handles`)
	...
	t.P(`func callRequestDeserialized(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirp"], `.ServerHooks) `, t.pkgs["context"], `.Context {`)
	t.P(`  if h == nil || h.RequestDeserialized == nil {`)
	t.P(`    return ctx`)
	t.P(`  }`)
	t.P(`  return h.RequestDeserialized(ctx)`)
	t.P(`}`)
	t.P()
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`    return`)
	t.P(`  }`)
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithRequestObject(ctx, reqContent)`)
	t.P()
	t.P(`  ctx = callRequestDeserialized(ctx, s.hooks)`)
	t.P()
