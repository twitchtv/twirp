...
func (t *twirp) generateUtils() {
	...
	t.P(`  ctx = callError(ctx, hooks, twerr) // Error hook`)
	t.P(``)
	...
	t.P(`  if writeErr != nil {`)
	t.P(`    // Most likely the connection is broken.`)
	t.P(`    // Logging the error is unacceptable because we don't have a user-controlled`)
	t.P(`    // logger; writing out to stderr without permission is too rude.`)
	t.P(`    // Calling the Error hook is unacceptable because it would trigger twice.`)
	t.P(`    // The best option is to ignore writeErr, the original err is being handled anyways.`)
	t.P(`    _ = writeErr`)
	t.P(`  }`)
	t.P(``)
	t.P(`  callResponseSent(ctx, hooks) // ResponseSent hook`)
	t.P(`}`)
	t.P()
	...
	t.P(`// handlePanics makes sure that panics during rpc methods respond with Twirp Interna`)
	t.P(`// error responses (status 500) and trigger hooks with the panic wrapped as an error.`)
	t.P(`// The panic is not intercepted and should be handled in middleware to avoid empty responses.`)
	t.P(`func handlePanics(ctx `, t.pkgs["context"], `.Context, resp `, t.pkgs["http"], `.ResponseWriter, hooks *`, t.pkgs["twirp"], `.ServerHooks) {`)
	t.P(`  if r := recover(); r != nil {`)
	t.P(`    err := errFromPanic(r) // allow to inspect error on Error hooks`)
	t.P(`    twerr := &internalWithCause{msg: "Internal service panic", cause: err} // avoid internal data on response`)
	t.P(`    writeError(ctx, resp, twerr, hooks) // write response and trigger Error hooks`)
	t.P(`    panic(r) // keep the panic going`)
	t.P(`  }`)
	t.P(`}`)
	t.P(``)
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  func() {`)
	t.P(`    defer handlePanics(ctx, resp, s.hooks)`)
