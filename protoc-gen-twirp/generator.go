...
func (t *twirp) generateUtils() {
	...
	t.P(`}`)
	t.P()
	t.P(`// Call twirp.ServerHooks.RequestDeserialized if the hook is available`)
	t.P(`func callRequestDeserialized(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirp"], `.ServerHooks, req interface{}) (`, t.pkgs["context"], `.Context, error) {`)
	t.P(`  if h == nil || h.RequestDeserialized == nil {`)
	t.P(`    return ctx, nil`)
	t.P(`  }`)
	t.P(`  return h.RequestDeserialized(ctx, req)`)
	t.P(`}`)
	t.P()
	t.P(`// Call twirp.ServerHooks.ResponseReady if the hook is available`)
	t.P(`func callResponseReady(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirp"], `.ServerHooks, resp interface{}) `, t.pkgs["context"], `.Context {`)
	t.P(`  if h == nil || h.ResponseReady == nil {`)
	t.P(`    return ctx`)
	t.P(`  }`)
	t.P(`  return h.ResponseReady(ctx, resp)`)
	t.P(`}`)
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  }`)
	t.P()
	t.P(`  ctx, err = callRequestDeserialized(ctx, s.hooks, reqContent)`)
	t.P(`  if err != nil {`)
	t.P(`    s.writeError(ctx, resp, err)`)
	t.P(`    return`)
	t.P(`  }`)
	...
	t.P(`  }`)
	t.P()
	t.P(`  ctx = callResponseReady(ctx, s.hooks, respContent)`)
	t.P()
