...
func (t *twirp) generateUtils() {
	...
	t.P(`// badRequestError is used when the twirp server cannot unmarshal a request`)
	t.P(`func badRequestError(cause error, msg string) `, t.pkgs["twirp"], `.Error {`)
	t.P(`	err := `, t.pkgs["twirp"], `.NewError(`, t.pkgs["twirp"], `.InvalidArgument, msg)`)
	t.P(`	err = err.WithMeta("twirp_invalid_request", cause.Error())`)
	t.P(`	return err`)
	t.P(`}`)
	t.P()
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`    s.writeError(ctx, resp, badRequestError(err, "failed to parse request json"))`)
	t.P(`    return`)
	t.P(`  }`)
	t.P()
...
func (t *twirp) generateServerProtobufMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`    s.writeError(ctx, resp, badRequestError(err, "failed to parse request proto"))`)
	t.P(`    return`)
	t.P(`  }`)
	t.P()
	t.P(`  // Call service method`)
