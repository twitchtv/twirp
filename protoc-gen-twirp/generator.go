...
func (t *twirp) generateUtils() {
	...
	t.P(`// asTwirpError ensures that the error is returned as a twirp.Error.`)
	t.P(`// If the error is a twirp.Error, returns the same error cated to the twirp.Error interface.`)
	t.P(`// If the error is wrapping a twirp.Error, returns the wrapped twirp.Error.`)
	t.P(`// If the error is NOT a twirp.Error, returns a new internal twirp.Error wrapping the original error.`)
	t.P(`func asTwirpError(err error) `, t.pkgs["twirp"], `.Error {`)
