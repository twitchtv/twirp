...
func (t *twirp) generateServerProtobufMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	if rpcType == download {
		t.P(`	// Call service method`)
		t.P(`	var respStream `, t.methodOutputType(method))
		t.P(`	func() {`)
		t.P(`		defer func() {`)
		t.P(`			// In case of a panic, serve a 500 error and then panic.`)
		t.P(`			if r := recover(); r != nil {`)
		...
		t.P(`		if err != nil {`)
		t.P(`			// TODO: figure out trailers' proto encoding beyond just a string`)
		t.P(`			if err == `, t.pkgs["io"], `.EOF {`)
		t.P(`				_ = trailer.EncodeStringBytes("OK")`)
		t.P(`			} else {`)
		t.P(`				_ = trailer.EncodeStringBytes(err.Error())`)
		t.P(`			}`)
		t.P(`			break`)
		t.P(`		}`)
		t.P(``)
