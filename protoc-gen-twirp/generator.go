...
func (t *twirp) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P()
	// dependency imports
	t.P(`import `, t.pkgs["protoio"], ` "github.com/gogo/protobuf/io"`)
...
func (t *twirp) generateUtils() {
	...
	t.P(`  }`)
	t.P()
	t.P(`  if err = `, t.pkgs["protoio"], `.NewFullReader(resp.Body, 0).ReadMsg(out); err != nil {`)
