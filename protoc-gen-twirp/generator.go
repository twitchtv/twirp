...
func (t *twirp) generateUtils() {
	...
	t.P(`// handlePanics makes sure that panics during rpc methods respond with`)
	t.P(`// Twirp Internal error responses (status 500) and trigger hooks with the panic wrapped as an error.`)
	...
	t.P(`    panic(r) // re-panic to keep standard recovery flow (http middleware or standard http.Server handling)`)
	t.P(`  }`)
	t.P(`}`)
	t.P(``)
